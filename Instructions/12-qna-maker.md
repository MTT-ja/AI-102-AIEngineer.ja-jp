...
2. 質問応答プロジェクトの **[ソースの管理]** ページで、 **[&#9547; ソースの追加]** の一覧にある **[おしゃべり]** を選択します。 次に、 **[おしゃべりの追加]** ダイアログ ボックスで、 **[親しみやすい]** を選択し、 **[おしゃべりの追加]** をクリックします。
...
1. **question** パラメーターの値を書き換え、スクリプトの Curl コマンドによって、**ラーニング パスとは何ですか?** という値が送信されるようにします。
...
1. ターミナル ペインで、スクリプトを実行するコマンド `ask-question.sh` を入力し、サービスによって返された JSON 応答を表示します。これには、*ラーニング パスとは何ですか?* という質問に対する適切な回答が含まれているはずです。
...
1. テスト チャット インターフェイスを使用して、ボットがナレッジ ベースから期待どおりに質問に回答することを確認します。 たとえば、`Microsoft 認定資格とは何ですか?` を送信してみます。
