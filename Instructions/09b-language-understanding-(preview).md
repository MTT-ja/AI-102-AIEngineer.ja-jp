...
2. ポータル上部の検索窓に、*[language]* と入力し、次の設定で**言語サービス** リソースを作成します。

    - **機能**:会話言語理解 (プレビュー) を含む、既定の機能を使用します。 
    - **[サブスクリプション]**:*ご自身の Azure サブスクリプション*
    - **リソース グループ**: *リソース グループを選択または作成します (制限付きサブスクリプションを使用している場合は、新しいリソース グループを作成する権限がないことがあります。提供されているものを使ってください)*
    - **リージョン**: 米国西部 2(West US 2) または西ヨーロッパ(West Europe)
    - **[名前]**: *一意の名前を入力します*
    - **価格レベル**: Free (F) または Standard (S) レベル (Free が利用できない場合) のいずれかを選択します
    - **責任ある AI 通知**: 同意
...
1. 左側のペインで、 **[トレーニング ジョブ]** ページを選択します。

2. **[トレーニング ジョブの開始]** ダイアログで、新しいモデルをトレーニングするオプションを選択し、**Clock** という名前を付けて、トレーニングが有効になっている場合に評価を実行するオプションを確認します。 

3. モデルのトレーニング プロセスを開始するには、 **[トレーニング]** をクリックします。

4. トレーニングが完了した場合 (数分かかることがあります)、ジョブの **[状態]** が **トレーニング成功** に変わります。

5. **[モデルのパフォーマンス]** ページを選択して、 **[Clock]** モデルを選択します。 全体と意図ごとの評価メトリック ( *精度* 、 *再現率* 、 *F1 スコア* ) と、トレーニング時に行った評価で生成された *混同行列* を確認します (サンプル発話数が少ないため、すべての意図が結果に含まれていない可能性がある点に注意してください)。
...
5. **Clock** モデルを選択し、 **[送信]** をクリックします。 このデプロイには時間がかかる場合があります。

6. モデルがデプロイされたら、 **[デプロイのテスト中]** ページで、 **[production]** デプロイを選択します。
...
5. **[スキーマ定義]** ページに戻り、 **[意図]** タブで **[GetDay]** 意図を選びます。
...
2. **[モデルのパフォーマンス]** ページを選択して、 **[Clock]** モデルを選択します。 次に、全体、エンティティごと、意図ごとの評価メトリック ( *精度* 、 *再現率* 、 *F1 スコア* ) と、トレーニング時に行った評価で生成された *混同行列* を確認します (サンプル発話数が少ないため、すべての意図が結果に含まれていない可能性がある点に注意してください)。

3. **[モデルのデプロイ]** ページで、 **[デプロイの追加]** を選択します。

3. **[デプロイの追加]** ダイアログで、**[既存のデプロイ名をオーバーライドする]** を選択し、「**production**」を選択します。

3. **Clock** モデルを選択し、 **[送信]** をクリックしてデプロイします。 これには時間がかかる場合があります。

4. モデルがデプロイされたら、 **[デプロイのテスト]** ページで **[production]** デプロイを選択して、それを次のテキストでテストします。
...
1. Language Studio の **[モデルのデプロイ]** ページで **[運用環境]** デプロイを選びます。 次に **[予測 URL の取得]** をクリックします。
...
1. Language Studio で、 **[質問と会話言語を理解する]** セクションまで下にスクロールし、 **[会話言語理解]** をクリックし、 **[Clock]** プロジェクトを選択します。 **Clock** をクリックしないで、円アイコンを選択して Clock プロジェクトを選択します。
